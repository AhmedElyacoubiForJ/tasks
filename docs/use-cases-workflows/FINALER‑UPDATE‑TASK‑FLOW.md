...
## Ziel
Dieser Flow beschreibt den vollständigen technischen und fachlichen Ablauf beim Aktualisieren einer bestehenden Task innerhalb einer TaskList.  
...
- Extraktion von `taskListId`, `taskId` und `UpdateTaskDto`
...
- `taskUpdater.applyFullUpdate(task, dto)` (PUT)
...
Er hält die Domain sauber von DTO‑Abhängigkeiten.
