package edu.yacoubi.tasks.controllers.api.v1.contract;

import edu.yacoubi.tasks.controllers.api.v1.contract.annotations.DomainErrorResponses;
...
@Tag(name = "B. Tasks – CRUD innerhalb von TaskList")
public interface ITaskListsTasksCrudApi extends IApiPrefix {
  ...
  ResponseEntity<APIResponseListTaskSummaryDto> getTasksByListId(
      @Parameter(description = "UUID der TaskList") @PathVariable("taskListId") UUID taskListId
  );
  ...
  @Operation(
      ...
      description =
          "Aktualisiert einen bestehenden Task innerhalb einer TaskList partiell (PATCH)."
  )
  @DomainErrorResponses
  @ApiResponse(
      responseCode = "200",
